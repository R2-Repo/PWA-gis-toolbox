...
describe('context-menu GIS tools', () => {
    it('evaluates feature deletable without throwing for spatial layers', () => {
        ...
        expect(isLayerFeatureDeletable({ type: 'spatial', storage: 'workspace' })).toBe(true);
        expect(isLayerFeatureDeletable({ type: 'spatial-chunked', storage: 'workspace' })).toBe(true);
