 ...
 */
export function measureKeyNotesTable(session, sheetId) {
    const notes = notesUsedOnSheet(session, sheetId);
...
export function drawSheetCalloutsOnPdf(doc, options = {}) {
    ...
    const leaders = leadersForSheet(session, sheetId);
    const notes = notesUsedOnSheet(session, sheetId);
    ...
    if (map && transform?.projectLngLat) {
    ...
    const size = measureKeyNotesTable(session, sheetId);
