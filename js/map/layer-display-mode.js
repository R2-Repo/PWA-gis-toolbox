...
export function resolveLayerDisplayMode(layer, mapEntry = null) {
    ...
    return {
        ...
        summary: 'The full layer is stored on this device. The map only draws features near the current view (up to a render cap), then updates as you pan and zoom.',
        details: [
            `About ${featureCount.toLocaleString()} features are stored in workspace (IndexedDB).`,
            `The map shows up to about ${RENDER_LIMITS.maxFeaturesPerSource.toLocaleString()} features in the current view at a time.`,
