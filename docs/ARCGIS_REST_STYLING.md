...
| Looks styled in ArcGIS Online, flat color here | Picture markers, full CIM, or Arcade label expressions we do not flatten |
...
- Not a full ArcGIS renderer: no hatch/picture fills, no marker-image libraries, no Arcade-driven labels.
- CIM symbols: only a flat color/width is extracted when present.
- Raster MapServer live layers do not use this path.
- Live Layers **catalog** styles are developer-authored ([`docs/LIVE_MAP_PRESETS.md`](LIVE_MAP_PRESETS.md)). Custom URL is the ad-hoc `drawingInfo` path.
